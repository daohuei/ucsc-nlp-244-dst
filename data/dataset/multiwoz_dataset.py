...
def parse_raw_belief(text):
    ...
    for field in text.split(','):
        ...
        slot_name = split[:3] if ' '.join(split[:3]) in SLOT_VALS else split[:2]
...
class HistoryBelief:
    ...
    def __init__(self, raw_text):
        ...
        self.text_fmt, n_subs = re.subn(r'<\|([^\|]*?)belief\|>.*<\|endof\1belief\|>', 
        ...
        self.context = re.search('<\|context\|>(.*)<\|endofcontext\|>', self.raw_text).group(1)
        # parse the belief in to an internal dict
        raw_belief = re.search('<\|belief\|>(.*)<\|endofbelief\|>', self.raw_text).group(1)
        self.belief = parse_raw_belief(raw_belief)
        # also parse the previous belief if it's there
        prev_belief_match = re.search('<\|previousbelief\|>(.*)<\|endofpreviousbelief\|>', self.raw_text)
        ...
        self.first_utterance = re.search('<\|user\|>([^<]*)<\|', self.raw_text).group(1)
    ...
    @property
    def text(self):
        return self.text_fmt.format(context=self.context, previousbelief=self.previous_belief_text, belief=self.belief_text)
...
class MultiWozDataset(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, filepath):
        ...
        padded_text_stream = itertools.chain.from_iterable([[EMPTY_HB], text_stream])
        # parse all lines with HistoryBelief class
        history_belief_generator = (HistoryBelief(raw_text.strip()) for raw_text in padded_text_stream)
        conversation = []
        _id = 1
