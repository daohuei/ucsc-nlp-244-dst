...
def random_mask_prev_beliefs(data, r):
    """randomly masks current belief values at proportion 0 <= r <= 1
    apply this after flatten_conversation"""
    hb = HistoryBelief(data["turn"])
    n = round(r * len(hb.prev_belief))
    mask_keys = random.sample(hb.prev_belief.keys(), n)
    for key in mask_keys:
        hb.prev_belief[key] = MASK_TOKEN
    return {"masked": hb.text, "target": data["turn"]}


def random_mask_both_beliefs(data, r):
    """randomly masks current belief values at proportion 0 <= r <= 1
    apply this after flatten_conversation"""
    hb = HistoryBelief(data["turn"])
    m = round(r * len(hb.prev_belief))
    n = round(r * len(hb.belief))

    mask_prev_keys = random.sample(hb.prev_belief.keys(), m)
    mask_keys = random.sample(hb.belief.keys(), n)

    for key in mask_prev_keys:
        hb.prev_belief[key] = MASK_TOKEN
...
def remove_repeating_masks(string):
    ...
    repeated_mask_idxs = [
        i
        for i, (tok1, tok2) in enumerate(pairwise(tokens), start=1)
...
def mask_context_belief_entities(data):
    ...
    hb = HistoryBelief(data["turn"])
    values_or = " | ".join(
        set(v for v in hb.belief.values() if v != "not mentioned")
    )
    if values_or:
