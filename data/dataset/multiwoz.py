...
class MultiWozDataset(object):
    def __init__(self, args, split="train", shuffle=True):
        ...
        if args.lexical:
            ...
            file_path = os.path.join(
                self.data_dir, "{}_dials.json".format(split)
            )

        if args.no_history:
            if args.lexical:
                input_word2index_name = (
                    "input_lang.word2index_lexicalized.json"
                )
                output_word2index_name = (
                    "output_lang.word2index_lexicalized.json"
                )
                input_index2word_name = (
                    "input_lang.index2word_lexicalized.json"
                )
                output_index2word_name = (
                    "output_lang.index2word_lexicalized.json"
                )
            else:
            ...
            if args.lexical:
                input_word2index_name = (
                    "history_lang.word2index_lexicalized.json"
                )
                output_word2index_name = (
                    "history_lang.word2index_lexicalized.json"
                )
                input_index2word_name = (
                    "history_lang.index2word_lexicalized.json"
                )
                output_index2word_name = (
                    "history_lang.index2word_lexicalized.json"
                )
        ...
        input_word2index_filepath = os.path.join(
            self.data_dir, input_word2index_name
        )
        output_word2index_filepath = os.path.join(
            self.data_dir, output_word2index_name
        )
        input_index2word_filepath = os.path.join(
            self.data_dir, input_index2word_name
        )
        output_index2word_filepath = os.path.join(
            self.data_dir, output_index2word_name
        )

        self.dialogues = json.load(open(file_path, "rt"))
        self.actions = json.load(
            open("resources/multi-woz/dialogue_acts.json", "r")
        )

        self.input_word2index = json.load(
            open(input_word2index_filepath, "rt")
        )
        self.output_word2index = json.load(
            open(output_word2index_filepath, "rt")
        )
        self.input_index2word = json.load(
            open(input_index2word_filepath, "rt")
        )
        self.output_index2word = json.load(
            open(output_index2word_filepath, "rt")
        )
        ...
        if args.lexical:
            cached_filename = "resources/cached_data_lexical_{}.pkl".format(
                split
            )
        else:
            cached_filename = "resources/cached_data_delex_{}.pkl".format(
                split
            )
    ...
    def process_belief_state(self, beliefs):
        ...
        for domain, slot, value in beliefs:
            if value == "not mentioned":
                continue
            concat.extend(
                [domain, slot] + value.strip().split(" ") + ["_SEP0"]
            )
        return " ".join(concat)
