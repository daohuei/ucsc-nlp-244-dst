    ...
    for domain in domains:
        ...
        for slot, value in bstate[domain]["book"].items():
            ...
            if value:
                ...
                raw_bstate.append(
                    (domain, new_slot, normalize_beliefstate(value))
                )
    # ipdb.set_trace()
    return raw_bstate
...
def createDelexData_mine():
    ...
    with open(
        os.path.join(DATA_DIR, "multi-woz/delex_mine.json"), "w"
    ) as outfile:
        json.dump(delex_data, outfile)
    ...
    if turn_id in actions[dial_name.split(".")[0]]:
        ...
        for k, v in turn_action.items():
            ...
            for (slot, value) in v:
                ...
                if (
                    domain in acts
                    and act in acts[domain]
                    and slot in acts[domain][act]
...
def divideData(data, lexicalize=False):
    ...
    trainListFile = open(
        os.path.join(DATA_DIR, "multi-woz/trainListFile"), "w"
    )
    ...
    for dialogue_name in tqdm(data):
        ...
        if dial:
            ...
            for turn_id, turn in enumerate(dial):
                ...
                turn_act_raw, turn_act = get_action(
                    actions, dialogue_name, turn_id + 1
                )
                # ipdb.set_trace()
            ...
            for dial_bstate in dialogue["bstate"]:
                for domain, slot, value in dial_bstate:
                    ...
                    belief_words.extend(
                        normalize_beliefstate(value).strip().split(" ")
                    )
...
def buildDictionaries(
    ...
    if lexicalize:
        ...
        input_index2word_filename = os.path.join(
            DATA_DIR, "input_lang.index2word.json"
        )
        input_word2index_filename = os.path.join(
            DATA_DIR, "input_lang.word2index.json"
        )
        output_index2word_filename = os.path.join(
...
def main():
    if sys.argv[1] == "delex":
        ...
        if not os.path.isfile(os.path.join(DATA_DIR, "multi-woz/delex.json")):
            ...
            data = json.load(
                open(os.path.join(DATA_DIR, "multi-woz/delex.json"))
            )
        ...
        if not os.path.isfile(os.path.join(DATA_DIR, "multi-woz/lex.json")):
            ...
            data = json.load(
                open(os.path.join(DATA_DIR, "multi-woz/lex.json"))
            )
